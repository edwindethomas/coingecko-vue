...
export default {
  ...
  data() {
    return {
      coins: [],
      filteredCoins:[],
      titles: ["#", "Coin", "Precio", "Precio cambiado", "24h volumen"],
      search:'',
    };
  },
  async mounted() {
  ...
  methods: {
    searchCoin(){
      this.filteredCoins = this.coins.filter(coin =>coin.name.toLowerCase().includes(this.search.toLowerCase()) || coin.symbol.toLowerCase().includes(this.search.toLowerCase()));
    }
  },
};
</script>
